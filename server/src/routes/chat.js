...
router.post("/chat", async (req, res) => {
  ...
  if (!keyResult) {
    return res.status(503).json({ error: "No API keys available. All keys have reached their daily limit or are in cooldown." });
  }
  ...
  try {
    ...
    console.error(`[Chat] Gemini API error (key ${index}):`, errorMsg);
